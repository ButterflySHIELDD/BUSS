...
contract ButterflySHIELD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BussDividendTracker public dividendTracker;
    address public marketingWallet = 0x455a2A20d3f7DfC48913162971AF8EE207F274f1;
    address public buybackWallet = 0x9A2CBA7d715b03Dd4b99b57C58b55f837DFd3103;
    ...
    uint256 public sellMarketingFee = 11;
    ...
    constructor() ERC20("Butterfly SHIELD", "BUSS") {
        dividendTracker = new BussDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "Butterfly SHIELD: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Butterfly SHIELD: The dividend tracker already has that address");

        BussDividendTracker newDividendTracker = BussDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Butterfly SHIELD: The new dividend tracker must be owned by the BUSS token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Butterfly SHIELD: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "Butterfly SHIELD: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "Butterfly SHIELD: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Butterfly SHIELD: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Butterfly SHIELD: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Butterfly SHIELD: This account cannot send tokens until trading is enabled");
        }
...
contract BussDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BussDividendTracker", "Buss_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BussDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BussDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main BUSS contract.");
    }
